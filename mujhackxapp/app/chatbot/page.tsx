...
import { Mic, MicOff } from 'lucide-react' // <-- Added Mic icons
...
export default function LoanAgentPage() {
  ...
  // Effect to play TTS when bot finishes replying
  useEffect(() => {
    if (
      prevIsLoadingRef.current && // Was loading
      !loading && // Is not loading anymore
      isVoiceModeActive // Voice mode is on
    ) {
  ...
  // This effect *must* come *after* any effects that read prevIsLoadingRef
  useEffect(() => {
  ...
  const fullReset = () => {
    ...
    stopVoiceMode() // Stop any recording
    setIsVoiceModeActive(false) // Turn off voice mode
    setMessages([])
    setStage('sales')
    setLoanData(null)
      ...
      socket.onmessage = (msg) => {
        ...
        try {
          ...
          if (transcript && transcript.trim().length > 0) {
            stopTTS() // <-- Interrupt TTS on speech
            ...
            if (received.is_final) {
              ...
              silenceTimerRef.current = setTimeout(() => {
                ...
                if (trimmed) {
                  handleSend(trimmed) // <-- Call the modified handleSend
                  accumulatedTranscript = ''
                  // setInput(''); // handleSend already does this
                }
              }, 2000) // 2-second silence delay
            } else {
  ...
  }, [connection, stopTTS, handleSend]) // <-- Add handleSend dependency
  ...
  }, [stopVoiceMode, stopTTS])

  // --- End Voice Mode Functions ---
    ...
    <>
      <SignedIn>
        ...
        <div className="bg-black min-h-screen flex">
              ...
              <SidebarBody className="justify-between gap-10 bg-[#1a1a1a] h-full">
                ...
                <SidebarLink
                  link={{
                    ...
                    href: '#',
                    icon: (
                      <div className="h-7 w-7 rounded-full bg-white text-black flex items-center justify-center text-sm font-bold">
          ...
          <div className={cn('flex-1 transition-all duration-300', sidebarOpen ? 'ml-[300px]' : 'ml-[60px]')}>
              ...
              <div className="border-t border-neutral-800 p-6">
                <div className="max-w-4xl mx-auto flex items-center gap-4 bg-neutral-900 rounded-2xl border border-neutral-800 px-6 py-4">
                  ...
                  <button
                    ...
                    className={cn(
                      'p-3 rounded-xl transition-all text-white',
                      isVoiceModeActive
                        ? 'bg-red-600 hover:bg-red-700' // Active recording state
                        : 'bg-neutral-800 hover:bg-neutral-700' // Inactive state
                    )}
                  ...
                  <button
                    onClick={() => handleSend()} // Pass no args
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={() => setShowSanction(false)}>
              <div className="bg-neutral-900 border border-neutral-800 rounded-2xl max-w-md w-full p-8 animate-in fade-in zoom-in duration-300" onClick={(e) => e.stopPropagation()}>
                <div className="text-center mb-6">
                  ...
                  {crmVerified && <p className="text-xs text-green-400 mt-2">[CRM Verified Customer]</p>}
                </div>

                <div className="bg-neutral-800/50 rounded-xl p-6 mb-6 space-y-3 text-white">
                  <div className="flex justify-between">
                    <span className="text-neutral-400">Amount</span>
                    <span className="font-bold">Rs. {sanctionedLoans[sanctionedLoans.length - 1].amount.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-neutral-400">Tenure</span>
                    <span className="font-bold">{sanctionedLoans[sanctionedLoans.length - 1].tenure} months</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-neutral-400">Rate</span>
                    <span className="font-bold">{sanctionedLoans[sanctionedLoans.length - 1].rate}% p.a.</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-neutral-400">EMI</span>
                    <span className="font-bold">Rs. {sanctionedLoans[sanctionedLoans.length - 1].emi.toLocaleString()}</span>
                  </div>
                </div>
